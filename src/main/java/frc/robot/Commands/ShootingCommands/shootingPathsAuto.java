...
public class shootingPathsAuto extends Command{
    ...
    public shootingPathsAuto(Shooter shooter, Drive drive, Indexer indexer,PathPlannerPath path,VisionSubsystem vision, Drive swerve) {
        this.drive =drive;
    ...
    public void initialize() {
        vision.ShootingMode(shooter.isShooting = true);
        timer.restart();
    }
    ...
    public void execute() {
        ...
        (velocityVector.getX()*swerve.getMaxLinearSpeedMetersPerSec(),
        velocityVector.getY()*swerve.getMaxLinearSpeedMetersPerSec(),
        omega);
        boolean isFlipped =
        ...
        if (!readyToShoot && (((shooter.isAtShootingVelocity(ShooterConstants.ShootingManualDistance) && shooter.isAtPivotPosition(ShooterConstants.ShootingManualDistance)) || Robot.shootingState.equals(ShootingState.PASSING)))  && (Math.abs(currentToWanted.getRadians()) < 10 && Robot.shootingState.equals(ShootingState.SHOOTING) || Math.abs(currentToWanted.getRadians()) < 8 && Robot.shootingState.equals(ShootingState.PASSING))) {
            readyToShoot = true;
